...
- un seul thème, noir sur blanc, surfaces beige (`#e3dccb`) ; les commandes sont posées sur cette
  surface haute, avec le texte encre (13,5:1) ; survol un ton plus soutenu (10,8:1) ; état pressé
  entièrement inversé, couleur de page sur couleur de texte ;
- fond, libellé et filet d'une commande dérivent tous de la paire fond/texte du thème ;
- bordure de 2 px sur les commandes : leur fond ne se détache du papier que de 1,2:1, c'est le filet
  qui les pose, à 16,1:1 ;
- le rose `#ffa6c1` ne sert plus qu'au repère "en cours", avec texte encre (10,1:1) : c'est la seule
  surface colorée de la liseuse, donc elle se repère sans la lire ;
- le repère "en cours" est un bloc rose plein, une teinte de surface serait imperceptible.

Contrastes mesurés, liseuse (cible 7:1) :

| Paire | Noir sur blanc |
|---|---|
| texte sur fond | 16,1 |
| texte atténué sur fond | 7,7 |
| libellé sur commande / survolée / pressée | 13,5 / 10,8 / 16,1 |
| titre sur bloc survolé | 10,8 |
| titre sur bloc en cours, au repos / survolé | 10,1 / 8,3 |
| bordure de commande sur fond (cible 3:1) | 16,1 |
...
| Indicateur `page-indicator` | `--indicator-size` (un quart du corps, entre 16 et 32 px), texte atténué, chiffres tabulaires, `aria-live="polite"`, aucun filet. Porte le pourcentage de progression (ex. "42 %") en lecture, "Titres 1 à 6 sur 7" sur la liste. Volontairement discret : c'est un état, jamais une commande |
...
| Aperçu `a-preview` | une phrase dans le palier choisi, à l'échelle de l'écran courant, avec la taille réelle sur son écran |
